...
def criar_tabela_cadastroJogos():
    conn = conectar_bd()
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS cadastroJogos (
...
class CadastroJogo():
    def __init__(self, titulo=str, descricao=str, preco=float, link=str):
        self.titulo = titulo
...
def cadastrar_usuario():
    ...
    if usuario and senha:
        ...
        if usuario_existe:
            ...
            janela_cadastro.grid_remove()  # Hide cadastro form
            janela_login.grid(row=0, column=0)  # Show login form
...
janela.grid_rowconfigure(0, weight=1)  # Configuração de peso para as linhas
janela.grid_rowconfigure(1, weight=1)
janela.grid_rowconfigure(2, weight=1)
janela.grid_rowconfigure(3, weight=1)
janela.grid_columnconfigure(0, weight=1)  # Configuração de peso para as colunas
